...
# Many teaching scripts still guard on ANTHROPIC_API_KEY before the first model
# call. Populate a harmless placeholder so those entry checks keep working.
...
def _ensure_openai_codex_auth(
    *, interactive: bool | None, force_login: bool
    ...
    if not interactive:
        raise RuntimeError(
            "OpenAI Codex OAuth login required. Run `uv run python login_openai_codex.py`."
        )
...
def _resolve_codex_url(base_url: str | None) -> str:
    ...
    # Legacy Anthropic/OpenRouter-style base URLs should not hijack the Codex flow.
...
def _friendly_error(status_code: int, raw: str) -> str:
    if status_code == 401:
        return "OpenAI Codex OAuth token is invalid or expired. Re-run `uv run python login_openai_codex.py`."
    if status_code == 403:
